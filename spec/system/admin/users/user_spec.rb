...
RSpec.describe '管理画面/ユーザー', type: :system do
  ...
  describe 'ユーザー一覧' do
    ...
    it 'ユーザーが一覧表示されること' do
      ...
      users.each do |user|
        ...
        expect(page).to have_content(I18n.l(user.created_at, format: :long)), 'ユーザーの作成日時が表示されていません'
      end
    end
    ...
    it '権限での検索が機能すること' do
      ...
      click_on '検索'
      expect(page).to have_content(admin.id), '権限での検索が正しく機能していません'
      expect(page).not_to have_content(general1.id), '権限での検索が正しく機能していません'
      expect(page).not_to have_content(general2.id), '権限での検索が正しく機能していません'
      expect(page).not_to have_content(general3.id), '権限での検索が正しく機能していません'
    end
  end
  ...
  describe 'ユーザー詳細' do
    ...
    it 'ユーザーの詳細情報が表示されること' do
      ...
      expect(page).to have_content(I18n.l(general1.created_at, format: :long)), 'ユーザーの作成日時が表示されていません'
    end
  end
